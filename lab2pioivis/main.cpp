...
#include <set>
#include <algorithm>
// обработать исключения и добавить чтобы все множество было в кортеже, а не в множестве
// ловить ошибки свяханные со скобками 
using namespace std;
...
struct Parser{
    ...
    various parse_element(){
        ...
        if (isdigit(input[pos]) || (input[pos] == '-' && pos+1 < input.size() && isdigit(input[pos+1]))) return parse_int();
    }
    various parse_set(){
        pos++;
        Set set;
...
int main(){
    ...
    inputFile.close();
    Parser parser;
    various MySet = parser.parse(stringvariant);
    Set inputSet = get<Set>(MySet.value);
